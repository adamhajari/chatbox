...
including a negative entry so a subject with no picture is asked about once.
    ...
    *question*; nothing checks the picture that comes back. Accepted for v1 only
    because Chatbox is family-only and supervised (D17). Before the kid pilot is
...
# looks exactly like being rate-limited. If Chatbox is ever published or used by
# another family (PLAN.md D17), put a real project URL or contact address here.
# https://meta.wikimedia.org/wiki/User-Agent_policy
USER_AGENT = ("Chatbox/0.1 (a family voice assistant for two children; "
              "https://www.mediawiki.org/wiki/API:Etiquette)")

# The screen as Chatbox mounts it: landscape, 320 wide by 240 tall (the panel is
