"""Runtime settings from chatbox.toml (provider, model names, file paths).

Anything that differs per machine — audio device names, GPIO pins, the web host —
belongs in `chatbox.local.toml` beside it, which git ignores. It is merged over the
...
class Settings:
    ...
    speech: SpeechSettings | None = None  # None when chatbox.toml has no [speech]
    ...
    local_config: Path | None = None  # the chatbox.local.toml that was merged in, if any
...
def _screen(data: dict, base: Path) -> ScreenSettings | None:
    """[screen] in chatbox.toml (the Pi's values belong in chatbox.local.toml).
...
# Every section chatbox.toml understands. A key outside one of these does nothing, and
...
def local_config_path(path: str | Path = "chatbox.toml") -> Path:
    """chatbox.toml -> chatbox.local.toml, beside it."""
    return Path(path).with_suffix(".local.toml")


def load_settings(path: str | Path = "chatbox.toml") -> Settings:
