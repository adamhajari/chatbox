...
def _start_web(args, settings, store: PolicyStore, counter, controls, log):
    ...
    if not args.web:
        return None
    from chatbox.web.app import WebServer
...
def _note_local_config(settings) -> None:
    ...
    if settings.local_config is not None:
        print(f"Settings: {settings.local_config.name} is layered over chatbox.toml.")
...
def _open_db(args, settings):
    ...
    if controls.paused:
        print("Chatbox is PAUSED (from the settings page): every question gets the resting reply.")
...
def cmd_chat(args, settings) -> None:
    ...
    print(f"Chatbox ({policy.persona.name}) · policy {policy.version_label()} · "
...
def cmd_talk(args, settings) -> None:
    ...
    from chatbox.audio.laptop import (Keyboard, KeyboardPress, LaptopSpeaker, PushToTalkMic,
                                      PushToTalkSettings)
    from chatbox.audio.light import LitSpeaker, NoLight
    ...
    if not sys.stdin.isatty():
        sys.exit("chatbox talk needs an interactive terminal (it listens for the spacebar)")
    if settings.speech is None:
        sys.exit("chatbox.toml has no [speech] section (see README: voice setup)")
    sp = settings.speech
    problem = audio_device_problem(sp.voice.get("input_device"),
                                   sp.voice.get("output_device"))
    if problem:
        sys.exit(f"chatbox talk needs a microphone and a speaker, but {problem}\n"
    ...
    show = _start_screen(settings, pipeline)
    if show is not None:
        from chatbox.audio.picture import PicturedSpeaker
...
def _start_screen(settings, pipeline):
    ...
    from chatbox.audio.picture import PictureShow, WatchingClassifier
...
def cmd_log(args, settings) -> None:
    if not settings.logging_enabled:
        print("Logging is off (chatbox.toml [logging] enabled = false).")
        return
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("chatbox.toml"),
                        help="settings file (default: ./chatbox.toml)")
