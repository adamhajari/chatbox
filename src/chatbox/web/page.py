...
from chatbox.policy import WEEKDAYS
from chatbox.web.form import KINDS
...
CANNED_HELP = {
    ...
    "something_went_wrong": ("Something went wrong", "When a check or the model fails. Chatbox "
...
def status_panel(status: dict[str, Any]) -> str:
    ...
    toggle = ('<button class="primary" name="action" value="resume">Resume</button>' if paused
              else '<button class="warn" name="action" value="pause">Pause Chatbox</button>')
...
</form><p class="help">Applies to the next thing Chatbox says. 0 is silent.</p></div>"""
...
<title>Chatbox settings</title><style>{_CSS}</style></head>
<body><main>
<h1>Chatbox settings</h1>
