"""The parent web UI (Phase 4): edit the policy of a running Chatbox.

It runs in the same process as `chatbox talk` / `chatbox chat` (started with `--web`),
...
from chatbox.limits import local_now
from chatbox.log import Controls, DailyCounter, ExchangeLog
from chatbox.pipeline import PAUSED_REPLY
...
from chatbox.web import form as F
from chatbox.web.page import render
...
def create_app(
    ...
    *,
    ...
    log: ExchangeLog | None = None,  # None = logging is off in chatbox.toml
    clock: Callable[[], datetime] = lambda: datetime.now(timezone.utc),
    lan_only: bool = True,
) -> FastAPI:
    """`counter` and `controls` are the ones the running pipeline uses; without them the
    page has no "Right now" panel."""
    app = FastAPI(title="Chatbox", docs_url=None, redoc_url=None, openapi_url=None)
    ...
    def page(data: dict, *, errors=None, message="", message_ok=True, preview: str | None = None,
        ...
        html = render(
            ...
            logging_hint="" if log is not None else (
                ...
                "[logging] enabled = true in chatbox.toml and restart Chatbox."),
    ...
    def show(saved: int | None = None, done: str | None = None) -> HTMLResponse:
        message = (f"Saved as version {saved}. Chatbox uses it from the next question."
        ...
        if action == "save":
            try:
                ...
                return page(data, message=f"Not saved: couldn't write the policy file ({e}). "
                            "Chatbox is still using the previous version.",
...
CONTROL_MESSAGES = {
    ...
    "resume": "Resumed. Chatbox answers from the next question.",
    "reset_count": "Today's question count is back to 0.",
    "set_volume": "Volume saved. It applies to the next thing Chatbox says.",
}


# ---- running it next to Chatbox -------------------------------------------------
...
def resolve_host(host: str) -> str:
    ...
    ip = lan_address()
    if ip is None:
        raise RuntimeError("couldn't find a home-network address; set [web] host in chatbox.toml")
    return ip


class WebServer:
    """uvicorn in a background thread of the Chatbox process."""
    ...
    def __init__(self, store: PolicyStore, host: str, port: int, *,
        ...
        self._thread = threading.Thread(target=self._server.run, name="chatbox-web", daemon=True)
        ...
        while not self._server.started:
            if not self._thread.is_alive() or time.monotonic() > deadline:
                raise RuntimeError(f"the parent web UI couldn't start on {self.host}:{self.port} "
                                   "(is the port in use? change [web] port in chatbox.toml)")
            time.sleep(0.05)
